...
void LAMMPSDataFileExporter::write(std::shared_ptr<System> system, std::string suffix) {
	...
	output_file << boost::format("%d atoms\n") % system->N();
	output_file << boost::format("%d bonds\n") % N_bonds;
	output_file << boost::format("%d atom types\n") % types.size();
	output_file << boost::format("1 bond types\n");
	...
	output_file << boost::format("%lf %lf xlo xhi\n") % box_low[0] % box_high[0];
	output_file << boost::format("%lf %lf ylo yhi\n") % box_low[1] % box_high[1];
	output_file << boost::format("%lf %lf zlo zhi\n") % box_low[2] % box_high[2];

	output_file << std::endl;
	output_file << "Masses" << std::endl << std::endl;

	for(auto type : types) {
		output_file << boost::format("%s 1 mass\n") % type;
	}
	...
	for(auto b : topology.bonds()) {
		output_file << boost::format("%d %d %d %d\n") % i % 1 % b[0] % b[1];
		i++;
	}
