...
void Topology::_fill_molecules(std::shared_ptr<System> system) {
	...
	if(_molecules.size() == 1) {
		...
		<< "Topology: Found 1 molecule";
	}
	else {
		BOOST_LOG_TRIVIAL(info)
		<< "Topology: Found " << _molecules.size() << " molecules";
	}
}
...
void export_Topology(py::module &m) {
	py::class_<Topology, std::shared_ptr<Topology>> topology(m, "Topology", R"pbdoc(
        ...
        Here the term ``topology`` refers to the way atoms/particles are partitioned into molecules. In its simplest form, a topology is just a list
	...
	topology.def("apply", &Topology::apply, py::arg("system"), R"pbdoc(
        Applies the current topology to the given system, adding the bonds to the particles and partitioning them into molecules.
